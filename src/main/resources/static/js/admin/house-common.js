...
function changeCity(city) {
    $.get('/house/address/cities', function (data, status) {
...
function changeRegion(region, cityName) {
    $.get('/house/address/regions?city_name=' + cityName, function (data, status) {
        if (status !== 'success' || data.code !== 200) {
            showError(data.message);
            return;
        }
...
function changeSubwayLine(subwayLine, cityName) {
    $.get('/house/address/subway/line?city_name=' + cityName, function (data, status) {
        if (status !== 'success' || data.code !== 200) {
            showError(data.message);
            return;
        }
...
function changeSubwayStation(subwayStation, subwayLineId) {
    $.get('/house/address/subway/station?subway_id=' + subwayLineId, function (data, status) {
        if (status !== 'success' || data.code !== 200) {
            showError(data.message);
            return;
        }
